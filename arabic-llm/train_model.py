...
DO NOT MODIFY: prepare_data.py, dataset files, or metadata

Goal: Minimize val_bpb (bits per byte) while maintaining quality metrics
"""
...
# Import fixed utilities (DO NOT MODIFY THIS IMPORT)
from prepare_data import (
    ...
    MAX_SEQ_LEN,
    ROLE_DISTRIBUTION,
    TIME_BUDGET_SECONDS,  # Fixed time budget (300 seconds = 5 minutes)
)
...
# DO NOT MODIFY - This ensures comparable experiments
# TIME_BUDGET_SECONDS = 300  # Imported from prepare_data.py
...
def train():
    """
    Main training loop with fixed time budget.
    
    The training will run for exactly TIME_BUDGET_SECONDS (5 minutes),
    regardless of architecture changes. This ensures fair comparison
        ...
        for batch in train_loader:
            ...
            if (step + 1) % LOG_INTERVAL == 0:
                ...
                print(f"Step {step+1}/{MAX_STEPS} | "
                      ...
                      f"Time: {elapsed/60:.1f}m/{TIME_BUDGET_SECONDS/60:.1f}m")
            ...
            if (step + 1) % EVAL_INTERVAL == 0:
                model.eval()
                with torch.no_grad():
                    val_bpb = compute_val_bpb(model, val_examples, tokenizer)
                    val_loss = compute_val_loss(model, val_examples, tokenizer)
                
                if val_bpb < best_val_bpb:
                    best_val_bpb = val_bpb
                    best_val_loss = val_loss
                    print(f"  ✓ New best val_bpb: {val_bpb:.4f} (val_loss: {val_loss:.4f})")
    ...
    # Final evaluation (CRITICAL: Always evaluate even if interrupted)
    total_time = time.time() - start_time
    model.eval()
    
    print("\nRunning final evaluation...")
    with torch.no_grad():
        final_val_bpb = compute_val_bpb(model, val_examples, tokenizer)
    ...
    baseline_val_bpb = 1.5  # Placeholder - load from previous experiment log
    improved = final_val_bpb < baseline_val_bpb
    
    # Log experiment (CRITICAL: Always log even if interrupted)
    log_experiment(
    ...
    print("=" * 70)
    print(f"Final val_bpb: {final_val_bpb:.4f} (lower is better)")
    ...
    return final_val_bpb, final_val_loss
...
if __name__ == "__main__":
    val_bpb, val_loss = train()
    
    # Exit with appropriate code
    # 0 = success, 1 = failure
    exit(0 if val_bpb > 0 else 1)
