...
class SemanticCache:
    ...
    def set(self, query: str, response: str) -> None:
        ...
        entry = CacheEntry(
            query=query,
            query_embedding=(
                query_embedding if query_embedding is not None else np.array([])
            ),
            response=response,
...
class ModelRouter:
    ...
    def get_usage_stats(self) -> Dict[str, Any]:
        ...
        distribution = {
            f"tier_{k}": v / total if total > 0 else 0 for k, v in self.usage.items()
        }
...
class CostTracker:
    ...
    def record(
        ...
        record = {
            ...
            "cached": cached,
            "query_hash": (
                hashlib.md5(query.encode()).hexdigest()[:8] if query else None
            ),
        }
    ...
    def get_report(self, since: Optional[datetime] = None) -> CostReport:
        """
