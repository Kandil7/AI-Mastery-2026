...
class ABTest:
    ...
    def get_variant(self, user_id: str) -> ModelVariant:
        ...
        hash_value = int(
            hashlib.md5(f"{user_id}:{self.test_name}".encode()).hexdigest(), 16
        )
    ...
    def analyze(self, metric: str = "error") -> Dict:
        ...
        if metric == "error":
            ...
            improvement = (np.mean(control_data) - np.mean(treatment_data)) / np.mean(
                control_data
            )
        else:
        ...
        return {
            ...
            "conclusion": {
                ...
                "recommendation": (
                    f"Deploy {winner}"
                    if is_significant and improvement > 0.02
...
if __name__ == "__main__":
    ...
    print(
        f"  Mean Error: {results['control']['mean']:.4f} ± {results['control']['std']:.4f}"
    )
    print(
        f"  95% CI: [{results['control']['ci_95'][0]:.4f}, {results['control']['ci_95'][1]:.4f}]"
    )
    print(f"  Samples: {results['control']['n']}")

    print(f"\nTreatment: {results['treatment']['name']}")
    print(
        f"  Mean Error: {results['treatment']['mean']:.4f} ± {results['treatment']['std']:.4f}"
    )
    print(
        f"  95% CI: [{results['treatment']['ci_95'][0]:.4f}, {results['treatment']['ci_95'][1]:.4f}]"
    )
    ...
    print(
        f"  Effect Size (Cohen's d): {results['statistical_test']['cohens_d']:.4f} ({results['statistical_test']['effect_size']})"
    )
