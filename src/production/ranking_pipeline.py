...
class PopularitySource(RetrievalSource):
    ...
    def retrieve(self, user: User, k: int) -> List[Candidate]:
        ...
        # Sort by popularity
        sorted_items = sorted(
            self.items, key=lambda x: self.popularity_scores.get(x.id, 0), reverse=True
        )
        ...
        for item in sorted_items[:k]:
            ...
            candidates.append(
                Candidate(item=item, retrieval_score=score, retrieval_source=self.name)
            )
...
class RecentSource(RetrievalSource):
    ...
    def retrieve(self, user: User, k: int) -> List[Candidate]:
        ...
        items_with_date = [
            (item, item.created_at or datetime.min) for item in self.items
        ]
...
class CollaborativeFilteringSource(RetrievalSource):
    ...
    def __init__(
        self, items: List[Item], user_item_matrix: Dict[str, Dict[str, float]]
    ):
        self.items = items
...
class PreRanker:
    ...
    def __init__(self, feature_dim: int, hidden_dim: int = 64, num_features: int = 10):
...
class FullRanker:
    ...
    def _build_model(self):
        ...
        # User tower
        self.user_W1 = np.random.randn(
            self.user_feature_dim, self.hidden_dims[0]
        ) * scale(self.user_feature_dim)
        self.user_b1 = np.zeros(self.hidden_dims[0])
        self.user_W2 = np.random.randn(self.hidden_dims[0], self.embedding_dim) * scale(
            self.hidden_dims[0]
        )
        self.user_b2 = np.zeros(self.embedding_dim)

        # Item tower
        self.item_W1 = np.random.randn(
            self.item_feature_dim, self.hidden_dims[0]
        ) * scale(self.item_feature_dim)
        self.item_b1 = np.zeros(self.hidden_dims[0])
        self.item_W2 = np.random.randn(self.hidden_dims[0], self.embedding_dim) * scale(
            self.hidden_dims[0]
        )
    ...
    def score(self, user: User, candidates: List[Candidate]) -> List[Candidate]:
        ...
        for candidate in candidates:
            # Encode item
            item_h1 = np.maximum(
                0, candidate.item.features @ self.item_W1 + self.item_b1
            )
...
class BusinessRuleReRanker:
    ...
    def add_boost_rule(
        self, rule: Callable[[Candidate], float], description: str = ""
    ) -> None:
        """
    ...
    def add_filter_rule(
        self, rule: Callable[[Candidate], bool], description: str = ""
    ) -> None:
        """
        Add a filter rule.
...
class RankingPipeline:
    ...
    def __init__(
        ...
        self.full_ranker = FullRanker(
            user_feature_dim=user_feature_dim, item_feature_dim=item_feature_dim
        )
    ...
    def add_retrieval_source(
        self, source: RetrievalSource, weight: float = 1.0
    ) -> None:
        """Add a candidate retrieval source."""
        self.candidate_generator.add_source(source, weight)

    def add_business_rule(
        self, rule_type: str, rule: Callable, description: str = ""
    ) -> None:
        """Add a business rule."""
        if rule_type == "boost":
    ...
    def rank(self, user: User, k: int = 20) -> RankingResult:
        ...
        candidates = self.candidate_generator.generate(
            ...
            // max(1, len(self.candidate_generator.sources)),
        )
...
def example_usage():
    ...
    pipeline.add_retrieval_source(
        EmbeddingSimilaritySource(items, item_embeddings), weight=1.0
    )
    pipeline.add_retrieval_source(PopularitySource(items, popularity), weight=0.5)
    pipeline.add_retrieval_source(RecentSource(items), weight=0.3)
    ...
    pipeline.add_business_rule(
        "boost",
        lambda c: (
            1.2 if c.item.metadata.get("price", 50) < 30 else 1.0
        ),  # Boost cheap items
