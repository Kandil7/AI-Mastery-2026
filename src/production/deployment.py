...
class ModelDeployer:
    ...
    def deploy_model(self, config: DeploymentConfig) -> str:
        ...
        with self.deployment_lock:
            deployment_id = (
                f"{config.model_name}-{config.model_version}-{int(time.time())}"
            )
            ...
            if not model_version:
                raise ValueError(
                    f"Model {config.model_name} version {config.model_version} not found in registry"
                )
    ...
    def _blue_green_deploy(
        self, config: DeploymentConfig, model_version: ModelVersion
    ) -> Dict[str, Any]:
...
class ModelDeploymentManager:
    ...
    def register_and_deploy(
        ...
        version_id = self.registry.register_model(
            model_name, model_path, metadata, accuracy, performance_metrics
        )
...
def deploy_model_simple(model_name: str, model_path: str, port: int = 8000) -> str:
    ...
    deployment_id = deployment_manager.register_and_deploy(
        model_name=model_name, model_path=model_path, config=config
    )
