...
class VectorDBBackupManager:
    ...
    def restore_backup(self, backup_id: str, target_path: Optional[str] = None):
        ...
        """
        backup_info = next(
            (b for b in self.metadata["backups"] if b["backup_id"] == backup_id), None
        )
    ...
    def apply_retention_policy(self, days: int = 30):
        ...
        logger.info(
            f"Retention policy applied: deleted {len(backups_to_delete)} old backups"
        )
...
def migrate_to_multi_tenant(
    old_db_path: str, new_db_path: str, default_tenant_id: str = "default"
):
    """
...
if __name__ == "__main__":
    ...
    backup_manager = VectorDBBackupManager(
        db_path="./vector_db", backup_path="./backups"
    )

    # Create full backup
    backup_id = backup_manager.create_backup(
        backup_type="full", description="Daily automated backup"
    )
    ...
    for backup in backup_manager.list_backups():
        print(
            f"  {backup['backup_id']}: {backup['size_mb']:.2f} MB ({backup['timestamp']})"
        )
