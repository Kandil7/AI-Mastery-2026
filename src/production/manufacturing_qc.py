...
class DefectDetector:
    ...
    def __init__(
        ...
        logger.info(
            f"DefectDetector initialized: {num_classes} classes, "
            ...
            for oc in range(out_channels):
                for i in range(h_out):
                    for j in range(w_out):
                        ...
                        j_start = j * stride
                        patch = x_padded[
                            n, :, i_start : i_start + kh, j_start : j_start + kw
                        ]
                        # Weighted sum
    ...
    def predict(
        self, image: np.ndarray, return_features: bool = False
...
class QualityInspectionPipeline:
    ...
    def inspect(
        self, image: np.ndarray, ground_truth: Optional[DefectType] = None
    ) -> InspectionResult:
        """
        ...
        if self.shadow_detector is not None:
            ...
            if shadow_type != defect_type:
                logger.debug(
                    f"Shadow model disagreement: {defect_type} vs {shadow_type}"
                )
    ...
    def _determine_action(
        self, defect_type: DefectType, confidence: float
    ) -> InspectionAction:
        """
    ...
    def _update_metrics(
        self, result: InspectionResult, ground_truth: Optional[DefectType]
    ):
        ...
        self.metrics.avg_latency_ms = (
            self.metrics.avg_latency_ms * (n - 1) + result.latency_ms
        ) / n
...
def create_inspection_system(
    connect_plc: bool = True, plc_host: str = "192.168.1.100", quantized: bool = True
    ...
    # Create detector
    detector = DefectDetector(
        num_classes=len(DefectType), input_size=(224, 224), quantized=quantized
    )
    ...
    pipeline = QualityInspectionPipeline(
        detector=detector, plc=plc, confidence_threshold=0.85, review_threshold=0.5
    )
...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        if i % 20 == 0:
            print(
                f"  Inspection {i}: {result.defect_type.value} "
