...
class QualityMonitor:
    ...
    def assess(self, query: str, response: str, context: List[str]) -> QualityScore:
        """
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "hallucination_rate": sum(
                1 for s in self.scores if s.hallucination_detected
            )
            / len(self.scores),
...
class RAGMetrics:
    ...
    def __init__(self):
        ...
        # Labels storage
        self.labeled_counters: Dict[str, Dict[str, int]] = defaultdict(
        ...
        for name, values in self.histograms.items():
            if values:
                ...
                for p in [50, 90, 95, 99]:
                    lines.append(
                        f'{name}{{quantile="{p/100}"}} {np.percentile(arr, p)}'
                    )
...
class RAGObservability:
    ...
    def __init__(
        ...
        self.quality = (
            QualityMonitor(llm_judge=llm_judge) if enable_quality_monitoring else None
        )
        self.latency = LatencyTracker()
    ...
    def record_retrieval(self, query: str, num_docs: int, latency_ms: float) -> None:
