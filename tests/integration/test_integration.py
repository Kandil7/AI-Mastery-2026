...
class TestNewtonCotes:
    ...
    def test_trapezoidal_linear_function(self):
        ...
        expected = 2.0  # âˆ«(2x+1)dx from 0 to 1 = [xÂ² + x]â‚€Â¹ = 2
        assert abs(result - expected) < 1e-10
    
    def test_trapezoidal_quadratic_convergence(self):
        """Trapezoidal should converge as O(1/NÂ²)."""
        f = lambda x: x ** 2
        true_value = 1.0 / 3  # âˆ«xÂ² from 0 to 1
    ...
    def test_simpsons_quadratic_exact(self):
        ...
        expected = 1.0/3 + 1 + 1  # âˆ«(xÂ² + 2x + 1)dx = 7/3
        assert abs(result - expected) < 1e-10
    
    def test_simpsons_quartic_convergence(self):
        """Simpson's should converge as O(1/Nâ´) for smooth functions."""
...
class TestGaussianQuadrature:
    ...
    def test_gauss_legendre_polynomial_exact(self):
        ...
        expected = 0  # âˆ«xâµ from -1 to 1 = 0 (odd function)
        assert abs(result - expected) < 1e-10
    ...
    def test_gauss_legendre_interval_transform(self):
        ...
        expected = 8.0 / 3  # âˆ«xÂ² from 0 to 2 = 8/3
        assert abs(result - expected) < 1e-10
    ...
    def test_gauss_hermite_standard_normal_variance(self):
        """Gauss-Hermite E[XÂ²] for X~N(0,1) should be 1."""
        f = lambda x: x ** 2
        result = gauss_hermite_expectation(f, mu=0, sigma=1, n=5)
        assert abs(result - 1.0) < 1e-10
    
    def test_gauss_hermite_general_normal(self):
        """Gauss-Hermite should work for general N(Î¼, ÏƒÂ²)."""
        mu, sigma = 3.0, 2.0
    ...
    def test_gauss_hermite_relu_expectation(self):
        ...
        # E[max(0, X)] for X~N(0,1) = 1/âˆš(2Ï€) â‰ˆ 0.3989
...
class TestMonteCarlo:
    ...
    def test_monte_carlo_gaussian_variance(self):
        """Monte Carlo should estimate E[XÂ²] = 1 for standard normal."""
        f = lambda x: x ** 2
        sampler = lambda n: np.random.randn(n)
    ...
    def test_monte_carlo_convergence_rate(self):
        """Monte Carlo error should decrease as O(1/âˆšN)."""
        f = lambda x: x ** 2
        sampler = lambda n: np.random.randn(n)
        ...
        # Ratio should be ~âˆš10 â‰ˆ 3.16
